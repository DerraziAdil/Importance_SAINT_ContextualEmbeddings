...
def set_random_seeds(seed=42):
    ...
    np.random.seed(seed)
    random.seed(seed)
    # XGBoost uses the 'random_state' parameter for reproducibility
...
def prepare_data(df, encoder=None):
    """
    Prepares the data for modeling by encoding categorical variables,
...
def plot_aggregated_shap(aggregated_shap, title_suffix, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'aggregated_SHAP_XGBoost_{title_suffix}.png'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'aggregated_signed_SHAP_XGBoost_{title_suffix}.png'))
    plt.close()
...
def perform_shap_analysis(model, X, feature_mapping, output_dir):
    ...
    # Access the preprocessor and transform the set
    ...
    plot_aggregated_shap(aggregated_shap, 'XGBoost', output_dir)
...
def perform_nested_cross_validation(X, y, param_grid, continuous_features, output_dir):
    ...
    - nested_cv_time (float): Time taken for nested cross-validation.
    ...
    nested_cv_time = nested_cv_end_time - nested_cv_start_time
    print(f"\nNested Cross-Validation Time: {nested_cv_time:.2f} seconds")
    ...
    # Extract the hyperparameters for XGBClassifier
    xgb_best_params = {k.replace('xgb__', ''): v for k, v in best_params.items()}
    ...
    # Calculate total training and testing time
    total_traintest_time = nested_cv_time + training_time_best_model
    ...
    return (
        ...
        best_val_y_val, 
        nested_cv_time,
        training_time_best_model,
        total_traintest_time
    )
...
def evaluate_model(model, X, y, dataset_type='Dataset'):
    ...
    # Generate classification report without accuracy
    report = classification_report(y, y_pred, zero_division=0, output_dict=True)
    report.pop('accuracy', None)  # Remove accuracy from the report
...
def plot_roc_curves(y_train, y_proba_train, y_test, y_proba_test, best_val_model, X_val, y_val, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'ROC_XGBoost.png'))
    plt.close()
...
def plot_performance_metrics(metrics_train, metrics_validation, metrics_test, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'Metrics_XGBoost.png'))
    plt.close()
...
def main():
    ...
    # Start the timer before nested CV and training
    total_start_time = time.time()

    # Perform nested cross-validation on the training data
    best_model, best_params, validation_scores, avg_validation_metrics, best_val_model, best_val_X_val, best_val_y_val, nested_cv_time, training_time_best_model, total_traintest_time = perform_nested_cross_validation(
    ...
    # End the total timer after training and testing completes
    total_end_time = time.time()
    total_traintest_time = total_end_time - total_start_time
    print(f"Total time for Training and Testing (Steps 11 & 12): {total_traintest_time:.2f} seconds.\n")
    ...
    # Save Metrics to JSON with separate timing entries
    metrics = {
        ...
        'nested_cv_time_seconds': nested_cv_time,
        'training_time_best_model_seconds': training_time_best_model,
        'total_traintest_time_seconds': total_traintest_time,
        'Best Hyperparameters': best_params
    }

    try:
        with open(os.path.join(output_dir, 'metrics_XGBoost.json'), 'w') as f:
            json.dump(metrics, f, indent=4)
        print("Saved performance metrics to 'metrics_XGBoost.json'.")
    except Exception as e:
    ...
    test_predictions.to_csv(os.path.join(output_dir, 'test_predictions_XGBoost.csv'), index=False)
    print("Saved predicted probabilities and true labels for the test set to 'test_predictions_XGBoost.csv'.\n")
